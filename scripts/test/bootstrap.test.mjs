...
test('profileNeedsSetup：缺文件 / 补丁内容变了都要重装', () => {
  ...
  fs.mkdirSync(path.join(profileDir, 'node_modules', '@company-desk', 'desk-image'), { recursive: true })
...
test('ensureProfile：写 manifest、链接插件与 dsh 回退目录（selfHeal=false）', () => {
  ...
  assert.match(manifest.dependencies['@company-desk/desk-image'], /^file:.*plugins\/desk-image$/)
