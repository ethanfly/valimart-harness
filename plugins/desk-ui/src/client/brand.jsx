 ...
 * 软件名 valimart harness；字标图里已有 VALIMART，无公司名时下面补 harness。
 */
...
/** 完整字标：原 VALIMART 图；无公司名时再补 harness。size 是字标图高度（px）。 */
export function Logotype({ size, tagline, compact = false }) {
  ...
  return (
    <span className={`dk-logotype${compact ? ' compact' : ''}`} style={{ '--dk-logo-h': `${h}px` }} title={PRODUCT_NAME}>
      ...
      {!compact && !tagline ? <small className="dk-logo-product">harness</small> : null}
