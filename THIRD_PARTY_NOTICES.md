...
| 组件 | 来源 | 许可证 | 说明 |
|---|---|---|---|
| `@deepseek-ai/dsh` 及子包（含 hoisted `zod`） | 内核前缀 / `scripts/kernel/pin.json` 锁定 0.1.3-alpha.2 | 以内核发布物为准 | Mixed 存储契约通过 junction 解析内核自带的 `zod`，插件目录不自带 `node_modules` |
| `dsh-better-sidebar@0.18.0` | 内核 profilePlugins | 上游包自带 | 与 Mixed 无关，随内核前缀分发 |
