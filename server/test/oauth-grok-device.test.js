...
test('Grok RFC 设备码：start 出 user_code；status 换到 access_token', async () => {
  ...
  assert.ok((st.models || []).some((m) => m.id === 'grok-4.6'))
  assert.equal(channels.store.load().items.grok?.credential, undefined)
  const done = await oauth.commit('grok', { id: 'u1', username: 'boss' }, { state: start.state, models: ['grok-4.6'] })
