...
test('ChatGPT 设备码：start 返回一次性代码；status 轮询后接入', async () => {
  ...
  assert.equal(st.json.status, 'success')
...
test('管理员 callback 成功：服务端存凭据，响应与列表不外泄', async () => {
  ...
  const done = await api('GET', `/api/channels/grok/oauth/status?state=${encodeURIComponent(ctx.start.state)}`, { token: ctx.boss.sessionToken })
  assert.equal(done.status, 200)
  assert.equal(done.json.status, 'success')
  assert.equal(done.json.channel.connected, true)
  assert.deepEqual(done.json.channel.models, ['grok-4.6', 'grok-4.6-fast'])
