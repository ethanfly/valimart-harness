/**
 * 公司盘本机镜像：对齐 plugins/desk-host/lib/drive-mirror.js。
 * 可见文件拉到本机；个人记忆区本地改动回推网关。
 */
import fs from 'node:fs'
import path from 'node:path'
import crypto from 'node:crypto'
import { assertInside } from './drive-paths.js'
    ...
    if (!fs.existsSync(readme)) {
      fs.writeFileSync(
        ...
        [
          ...
          '',
          '本目录由 valimart harness VS Code 插件与公司网关同步。Agent 用 company_memory_write 写记忆，用 company_task_attach 把产物挂到任务卡。',
          '',
        ].join('\n'),
      )
    }
...
export function makeDriveGateway(client) {
  const token = () => client.store.data.sessionToken
  return {
    get(apiPath) {
      const file = String(apiPath).includes('/api/drive/file?')
      return client.request('GET', apiPath, {
        token: token(),
        responseBuffer: file,
        timeoutMs: file ? 120_000 : 60_000,
      })
    },
    put(apiPath, body, opts = {}) {
      return client.request('PUT', apiPath, {
        token: token(),
        raw: true,
        body,
